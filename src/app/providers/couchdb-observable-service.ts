...
// import { COUCH } from './conf/couchdb-service-conf';

// private header for proxies requesting verification...
// const PROXY_HEADER = new HttpHeaders({'Couch': 'couchdb-request'});
...
@Injectable({providedIn: 'root'}) export class CouchDbService {
	...
	couchLogin(name, password): Observable<any>  {
		return this.http.post(this.db_url + '_session', {name: name, password: password}, {observe: 'body', responseType: 'json', withCredentials: true});
	}
